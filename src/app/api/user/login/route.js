...
export async function POST(req,res){
    ...
    try {
        ...
        if(matchedPassword){
             userDetail = await User.findOne({email:email},{__v:0}).select("-password -confirmPassword -createdAt -updatedAt")
        }
        ...
        const response =  NextResponse.json(userDetail,{message:"User Logged In Successfully"},{status:200})
