...
import { updateOther,Discard } from "../../../../../../../services/apiService/pasteurization/pasteurization";
    ...
    <div className="mx-4">
      <FormBorder title={"Other Test Result"}>
        <div className="flex justify-end">
          <button className="bg-red-600 rounded-md shadow-md text-white px-4 py-2 " onClick={()=>handleDiscard()} >Discard</button>
        </div>
        <form action="" onSubmit={handleSubmit((data) => onSubmit(data))}>
          <div className=" flex  flex-col gap-3">
            ...
            <div className="flex justify-end" >
              <button className="bg-indigo-600 text-white rounded-md px-4 py-2" onClick={(e) => handleAppend(e)}>
                Add
              </button>
            </div>
            ...
            {fields?.map((field, index) => {
              return (
                <div key={field.id} className="flex items-center gap-3 justify-between">
                  ...
                  <input type="text" className="inputStyle" {...register(`other.${index}.testName`)} placeholder="Test Name" />
                  </div>
                  <div className="flex flex-col w-full">
                    <label htmlFor="">
                      Other Test Result <span className="text-red-600">*</span>{" "}
                    </label>
                    <select className="inputStyle" {...register(`other.${index}.testResult`)}>
                      <option value="" selected disabled>
                        --Select Result--
                      </option>
                  ...
                  </div>
                  {
                    ...
                    fields?.length > 1 &&
                    <div>
                      <button className="bg-red-600 text-white rounded-md shadow-md px-3 py-2" onClick={()=>remove(index)} >
                        Remove
                      </button>
                    </div>
                  }
