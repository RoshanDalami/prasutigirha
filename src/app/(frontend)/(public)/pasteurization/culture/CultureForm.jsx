...
export default function CultureForm() {
  const { data: poolingResult = {} } = usePasteurizationList(1, 500);
  ...
  const dataForExcel = poolingList?.map((item) => ({
    Pooling_Condition: gestationalAgeList?.map((items) => {
      if (item.poolingCondition === items.gestationalId) return items.gestationalName;
    }),
    ...
    <div className="px-6">
      ...
      >
        <div className="flex flex-col items-center">
          <h1 className="text-xl font-bold">Paropakar Maternity and Women&apos;s Hospital</h1>
          <h1 className="text-xl font-bold">Department of Pediatrics</h1>
          <h1 className="text-xl">Comprehensive Lactation Management Center (CLMC)</h1>
          <h1>Thapathali, Kathmandu</h1>
          <h1 className="font-bold text-md underline">Pre-pasteurization Culture / Post-pasteurization Culture</h1>
        </div>
        <div className="mt-4">
              ...
              if (item.culture != null) {
                return (
                  <tr key={index} className={`${item.culture ? "bg-rose-600/50" : "bg-lime-600/50"}`}>
                    <td className="border border-black font-bold px-2 py-3">{index + 1}</td>
                    <td className="border border-black font-bold px-2 py-3">
                      <Link href={`/pasteurization/pasteurizationList/${item._id}`}>{item.batchName}</Link>
                    </td>
                    <td className="border border-black font-bold px-2 py-3">{item.date}</td>
                    <td className="border border-black font-bold px-2 py-3">
